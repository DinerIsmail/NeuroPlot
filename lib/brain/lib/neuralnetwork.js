...
var NeuralNetwork = function(options, io) {
  options = options || {};
  ...
  train: function(data, options) {
    ...
    for (var i = 0; i < iterations && error > errorThresh; i++) {
      ...
      if (log && (i % logPeriod == 0)) {
        ...
        if (i == 0) {
          socketObj.sockets.emit('draw-vis-initial', {
      ...
      if ((i != 0 && i % 1000 == 0) || (i == iterations-1 || error < errorThresh)) {
        socketObj.sockets.emit('refresh-graph-live', {
          "errorData": errorGraphArray,
          "nnSpec": this.toJSON()
        });
        errorGraphArray = [];
