...
var NeuralNetwork = function(options, socket) {
  options = options || {};
...
NeuralNetwork.prototype = {
  ...
  train: function(data, options) {
    ...
    // Added below for live refresh of nn visualisation
    var refreshVisWhenLogging = options.refreshVisWhenLogging || false;

    var inputSize = data[0].input.length;
    var outputSize = data[0].output.length;

    // Added to aid in visualising the decrease in global error
    var errorsArray = [];
    var errorGraphArray = [];
    ...
    for (var i = 0; i < iterations && error > errorThresh; i++) {
      ...
      if (log && (i % logPeriod == 0)) {
        ...
        errorGraphArray.push({ "iterations": i, "error": error });

        if (i == 0) {
          currentSocket.emit('draw-vis-initial', {
            "errorData": errorGraphArray,
            "nnSpec": this.toJSON()
          });
        }
      }

      if (refreshVisWhenLogging && (i != 0 && i % 1000 == 0) || (i == iterations-1 || error < errorThresh)) {
        currentSocket.emit('refresh-graph-live', {
          "errorData": errorGraphArray,
          "nnSpec": this.toJSON()
        });
  ...
  toJSON: function() {
    ...
    for (var layer = 0; layer <= this.outputLayer; layer++) {
      ...
      for (var j = 0; j < nodes.length; j++) {
        var node = nodes[j];
        layers[layer][node] = {};
        layers[layer].size = nodes.length;

        if (layer > 0) {
          layers[layer][node].bias = this.biases[layer][j];
          layers[layer][node].weights = {};
          layers[layer][node].output = this.outputs[layer][node];
